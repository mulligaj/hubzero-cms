...
if ($this->resource->type == 4) {
	...
	else if (is_file(JPATH_ROOT . DS . ltrim($url, DS))) 
	{
		if (strtolower($type) == 'swf') {
			...
			{
				$sef = JRoute::_('index.php?option=com_resources&id='.$this->activechild->id.'&task=download&file='.basename($this->activechild->path));
