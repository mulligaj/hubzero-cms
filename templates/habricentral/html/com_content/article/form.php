...
<div class="main section edit-form">
<?php
$action = $this->action;
if (strstr($this->action,'task=edit')) {
	//$bits = explode('?',$this->action);
	//$action = $bits[0];
	$action = str_replace('task=edit&','',$this->action);
}
?>
<form action="<?php echo $action; ?>" method="post" name="adminForm" id="hubForm" class="full" onSubmit="setgood();">
	...
	<p class="submit" style="margin-bottom: 5em">
		<button type="button" onclick="submitbutton('save')">
			<?php echo JText::_('Save') ?>
		</button> &nbsp; 
		
		<a href="<?php echo '#'; //$action; ?>" onclick="submitbutton('cancel')"><?php echo JText::_('Cancel') ?></a>
	</p>
...
if (!$ref) {
	$ref = base64_decode(JRequest::getVar('ret', '', 'get'));
}
?>
