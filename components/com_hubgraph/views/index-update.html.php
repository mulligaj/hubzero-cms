...
<form id="search-form" class="search" action="" method="get">
	<div class="bar">
		<input type="text" autocomplete="off" name="terms" class="terms" placeholder="Enter search terms" value="<?= a($req->getTerms()) ?>" />
		<a class="clear" href="<?= preg_replace('/[?&]+$/', '', $base.($_SERVER['QUERY_STRING'] ? '?'.preg_replace('/^&/', '', preg_replace('/&?terms=[^&]*/', '', urldecode($_SERVER['QUERY_STRING']))) : '')) ?>">x</a>
